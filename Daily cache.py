...
v17 (2026-01-08):
- Adds the missing computations requested by Karanveer:
  * Bollinger Bands (20): percent B and bandwidth
  * Yang–Zhang volatility (rolling windows 20 & 50)
  * Donchian channel position (20 & 50)
  * Breakouts (20 & 50) and days-since (20)
  * Dollar volume & rolling Z-scores (20/50/252) + volume surge flags
  * Day-of-week code and one-hot dummies
- Keeps v16 changes: removes RSI×ADX bucket crosses; saves numeric CPR & structure codes; drops raw strings.

This file includes one additional fix:
- KiteProvider.fetch_daily(...) now auto-chunks >2000-day windows into <=1999-day sub-requests and merges them.
"""
...
# -------------------- Schema + .ok metadata --------------------
SCHEMA_VERSION = 17  # bumped after adding requested computations
...
UNRESOLVED_SYMBOLS_LOG = Path(os.environ.get("UNRESOLVED_SYMBOLS_LOG")
                              or str(_instrument_cache_path().parent / "unresolved_symbols.jsonl"))
OVERRIDES_FILE = Path(os.environ.get("SYMBOL_OVERRIDES_FILE")
    ...
    def _build_maps(self):
        ...
        for r in rows:
            ...
            if not itok or not ts:
                continue
            score = 2 if (seg.upper().startswith(("NSE", "BSE")) and inst_type.upper() == "EQ") \
                    else 1 if seg.upper().startswith(("NSE", "BSE")) else 0
...
class KiteProvider:
    ...
    def _load_token(self) -> None:
        ...
        for attempt in range(max_tries):
            try:
            ...
            except (ReadTimeout, ConnectTimeout, RequestsConnectionError, Urllib3ReadTimeoutError, socket.timeout) as e:
                last_err = e
                ...
                last_err = e; break
        if last_err:
            raise RuntimeError(f"Kite API connectivity failed after retries: {last_err}") from last_err
    ...
    def _symbol_to_instrument_token(self, symbol: str) -> int:
        ...
        if sym in self._instruments: return self._instruments[sym]
        ...
        for r in rows:
            ...
            seg = str(r.get("segment", ""))
            inst_type = str(r.get("instrument_type", ""))
            if not itok or not ts: continue
            score = 2 if (seg.upper().startswith(("NSE", "BSE")) and inst_type.upper() == "EQ") \
                else 1 if seg.upper().startswith(("NSE", "BSE")) else 0
            prev = by_exact.get(ts)
            if (prev is None) or (score > prev[0]): by_exact[ts] = (score, int(itok))
            base = ts[:-3] if ts.endswith("-EQ") else ts
            prevb = by_base.get(base)
            if (prevb is None) or (score > prevb[0]): by_base[base] = (score, int(itok))
        token = None
        if sym in by_exact: token = by_exact[sym][1]
        elif f"{sym}-EQ" in by_exact: token = by_exact[f"{sym}-EQ"][1]
        elif sym in by_base: token = by_base[sym][1]
        else:
        ...
        if token is None:
            ...
            if resolved is not None: token = int(resolved)
        if token is None:
    ...
    # ---------- ONLY CHANGE: auto-chunked daily fetch to respect Kite's 2000-day max ----------
    def fetch_daily(self, symbol: str, start: dt.date, end: dt.date) -> pd.DataFrame:
        """
        Fetch daily candles for [start, end] inclusive, automatically chunking the request
        to respect Kite's 2000-day per-call maximum. Merges and returns a normalized DataFrame.
        """
        MAX_DAYS = 1999  # keep well under server limit
        ...
        for chunk_start, chunk_end in _iter_chunks(start, end):
            start_dt = dt.datetime.combine(chunk_start, dt.time(0, 0))
            ...
            try:
                ...
                # Re-resolve once, then retry this chunk
                sym = symbol.strip().upper()
...
def compute_vpoc(df: pd.DataFrame, bins: int = 50) -> float:
    ...
    if not math.isfinite(lo) or not math.isfinite(hi): return float("nan")
    if math.isclose(lo, hi): return float(lo)
...
def _period_trend_from_highs_leakfree(ts: pd.Series, high: pd.Series, period: str) -> pd.Series:
...
# --------- Indicators list (pre-declare for dtype stability) ----------
DAILY_INDICATOR_COLUMNS = [
    ...
    # Numeric code columns (saved)
    "D_tmr_cpr_vs_today_code","D_cpr_vs_yday_code","D_structure_trend_code",
    # Added computations
    "D_dow","D_bb_pctB_20","D_bb_bw_20","D_vol_yz_20","D_vol_yz_50",
...
def compute_daily_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Weekly VPOC (leak‑free running)
    df["D_weekly_vpoc"] = np.nan
    price_profile = pd.DataFrame({"high": high, "low": low, "close": close, "volume": volume})
    weekly_periods = timestamps_local.dt.to_period("W-FRI")
    for _, g in price_profile.groupby(weekly_periods, sort=True):
        if g.empty: continue
        run_vals = []
        for i in range(len(g)):
            vp = compute_vpoc(g.iloc[:i+1])
            run_vals.append(vp)
        df.loc[g.index, "D_weekly_vpoc"] = run_vals
    ...
    rel_tmr = _cpr_relationship(cpr_bc_y, cpr_tc_y, df["D_tmr_cpr_bc"], df["D_tmr_cpr_tc"])
    rel_vs_y = _cpr_relationship(cpr_bc_y, cpr_tc_y, df["D_cpr_bc"], df["D_cpr_tc"])
    ...
    df["D_weekly_trend"] = _period_trend_from_highs_leakfree(timestamps, high, "W-FRI")
    df["D_monthly_trend"] = _period_trend_from_highs_leakfree(timestamps, high, "M")
    ...
    # Numeric codes (Int8) — NO NaNs
    def _encode_rel(s: pd.Series) -> pd.Series:
        return s.map({"Above": 1, "Inside": 0, "Overlap": 0, "Below": -1}).fillna(0).astype("Int8")
    def _encode_trend(s: pd.Series) -> pd.Series:
        return s.map({"uptrend": 1, "range": 0, "downtrend": -1}).fillna(0).astype("Int8")
    ...
    # ===== Added computations =====
    # Day-of-week code and one-hot dummies
    ts_local = timestamps.dt.tz_convert(None) if timestamps.dt.tz is not None else timestamps
    ...
    # Bollinger Bands (20): %B and bandwidth
    sma20 = df["D_sma20"]
    std20 = close.rolling(20, min_periods=1).std()
    bb_upper = sma20 + 2 * std20
    bb_lower = sma20 - 2 * std20
    width = (bb_upper - bb_lower)
    df["D_bb_pctB_20"] = ((close - bb_lower) / width.replace(0, np.nan)).clip(lower=-5, upper=5)
    df["D_bb_bw_20"] = (width / sma20.replace(0, np.nan)).replace([np.inf, -np.inf], np.nan)

    # Yang–Zhang volatility (daily variance, then rolling sqrt mean)
    ...
    rs = (log_h_o * log_h_c + log_l_o * log_l_c)  # Rogers–Satchell per-day
    k = 0.34
    ...
    # Donchian channel position (0..1)
    def _donch_pos(n: int):
        hi_n = high.shift(1).rolling(n, min_periods=1).max()  # previous window to avoid same-day bias
        lo_n = low.shift(1).rolling(n, min_periods=1).min()
        rng_n = (hi_n - lo_n)
        return ((close - lo_n) / rng_n.replace(0, np.nan)).clip(lower=-1, upper=2)
    df["D_donch_pos_20"] = _donch_pos(20)
    df["D_donch_pos_50"] = _donch_pos(50)
